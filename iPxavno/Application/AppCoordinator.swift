    ...
    private func refreshContentCatalog() {
        Task {
            do {
                ...
                container.analytics.record(
                    AnalyticsEvent(name: "content_catalog_refreshed", properties: ["cards": "\(cards.count)"])
                )
            } catch {
                container.analytics.record(
                    AnalyticsEvent(name: "content_catalog_refresh_failed", properties: ["reason": error.localizedDescription])
                )
            }
        }
    }
